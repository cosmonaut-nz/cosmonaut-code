...
impl Config {
    pub fn load() -> Self {
        ...
        let openai_org_id = env::var("OPENAI_ORGANIZATION_ID").expect("OPENAI_ORGANIZATION_ID must be set");
        let openai_org_name = env::var("OPENAI_ORGANIZATION_NAME").expect("OPENAI_ORGANIZATION_NAME must be set");
        ...
        Config { openai_org_id, openai_org_name, openai_api_key, openai_model, folder_path, review_type }
    }
}
